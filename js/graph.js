...
// Supabase 클라이언트 인스턴스를 담을 전역 안전 변수
let _supabase = null;
...
const TaraeStorage = {
     ...
     * 이 방식을 통해 깃허브 코드상에 비밀 키가 노출되는 것을 원천 차단합니다.
     */
    async init() {
    ...
    // 로그인된 유저의 고유 UUID 추출 (PostgreSQL RLS 데이터 격리용 고유 식별자)
    async getUserId() {
        await this.init();
    ...
    // 칸반보드에서 카드를 드래그 또는 화살표로 이동시켰을 때 JSON 데이터 실시간 업데이트
    ...
    // ==========================================
    // 🌌 3. 타래장 기억 저장소 (Vault) 비즈니스 로직
    // ==========================================

    // 타래장 은하수 그래프 및 리스트 뷰 렌더링을 위한 사용자의 생각 파편 전체 조회
    async getThoughts() {
        await this.init();
        const user_id = await this.getUserId();
        if (!user_id) return { data: null, error: new Error("인증 세션이 만료되었습니다. 로그인이 필요합니다.") };
